...
코드/배선은 끝났고, 아래는 에셋(애니메이션/파티클) 쪽에서 아직 안 고친 것들:
- **Slash Attack(검풍) 애니메이션 교체 필요**: 스킬 쓰는 동안 이상한 곳을 바라봄 — 지금까지 폐기했던 Overhead Slash/ComboAttack/Leaping Strike와 같은 원인(클립 자체에 정면이 아닌 방향으로 도는 동작이 baked pose로 박혀있음, Import 설정으로 못 고침)일 가능성이 높다. 다른 클립을 찾아서 교체할 것 — 새 클립 고를 때는 유니티로 가져온 뒤 Scene 뷰 기즈모나 Curves 뷰로 미리 확인할 것(위쪽 "이 문서와 코드에 대해" 관련 세션에서 나온 팁).
- **`Boss_Slash_1/2/3` 파티클 유지시간 부족**: 투명해지면서 일찍 사라지는 느낌 — 파티클 자체의 Duration/Lifetime 값과 `Boss_Attack_Effect_Projectile`의 `travelTime`을 늘려서 맞출 것.

보스 전투 기능이 다 완성된 다음에 할 계획(순서상 나중):
- **패턴별 피격(타격) 파티클 추가**: 지금은 발사/스윙 이펙트만 있고, 실제로 맞는 순간의 임팩트 이펙트는 각 패턴마다 따로 없음. 완성 후 하나씩 붙일 것.
- **플레이어 피격 처리 구현**: `Player.cs`의 `MonsterAttack` 레이어 `OnTriggerEnter`가 아직 빈 스텁(케이스만 있고 실제 처리 없음)이라, 지금은 보스 공격이 다 맞아도 플레이어 체력이 안 깎인다. `Monster_HitDamage`가 플레이어 공격 받는 방식(레이어 감지 → 상대방 컴포넌트의 데미지 계산 함수 호출 → 자기 체력 차감, "데미지 전달 패턴" 참고)과 동일하게 구현할 것.

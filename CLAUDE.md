...
"남은 작업"/"완료된 작업" 형태로 진행 상황을 기록하는 섹션(예: "OrcKing 보스 - 남은 작업")을 다룰 때 지킬 것:

- **순서 고정**: 남은 작업 → 완료된 작업. "남은 작업" 섹션 바로 아래에 "완료된 작업" 섹션을 만들어둔다(현재 비어있어도 미리 생성).
- **진행중 표시**: 며칠 걸리는 항목은 "남은 작업"에서 내용 뒤에 `(진행중)`을 붙인다. 표시 없으면 아직 시작 전. 목적은 "남은 작업"에 아직 구현 안 한 항목이 여러 개 섞여있을 때 지금 실제로 뭘 작업 중인지 구분하기 위함 — 표시가 없으면 AI가 여러 항목 중 뭘 도와야 할지 판단할 수 없다.
  - **세션 시작 시** "남은 작업"에 `(진행중)` 표시가 하나도 없으면, AI가 날짜만 보고 임의로 판단하지 말고 먼저 "지금 뭐 작업 중이야?"라고 물어본 뒤 답변에 맞춰 Edit으로 표시를 붙인다.
- **완료 항목 표기**: 상세 설명 없이 카테고리 + 짧은 기능 키워드로만 적는다. 예: `- 스킬 (MM/DD/YY): 슬롯 UI, 쿨타임 표시, 아이콘 연동`. 상세 구현 내용은 기록하지 않는다(코드가 진실의 원천이므로 나중에 코드 리뷰로 확인).
  - **카테고리는 스크립트/매니저 이름이 아니라 도메인 단위**로 쓴다(`플레이어`, `스킬`, `몬스터`, `보스`, `UI` 등 — "폴더 구조"에서 쓰는 단위와 동일). 매니저 여러 개에 걸친 기능도 유저가 보는 도메인 하나로 묶는다.
  - **같은 카테고리가 여러 날짜에 걸쳐 완료되면 기존 줄에 합치지 말고 새 줄로 따로 추가**한다(기존 줄을 찾아 고치는 대신 항상 맨 아래에 추가만 하면 되게).
- **항목이 끝나면 2단계로 나눠서 처리한다.** 먼저 "완료된 작업"에 위 형식으로 추가하고, 그다음 별도 Edit로 "남은 작업"에서 그 줄만 제거한다. 두 동작을 하나의 Write로 합쳐서 처리하지 않는다.
- **새 할 일을 추가할 때**는 "남은 작업" 맨 아래에 `- MM/DD/YY: 내용` 형식으로 추가한다.
- **긴 설명이 필요해도 별도 섹션으로 분리하지 않고, "남은 작업" 항목 자체에 필요한 만큼 길게 적는다.** 참조("상세 내용은 O 섹션 참고")로 쪼개면 완료 시 섹션 간 연결이 끊길 위험이 생기는데(아래 "기존 레거시 항목" 참고), 항목 안에 다 적어두면 그 항목을 통째로 "남은 작업"→"완료된 작업"으로 옮기기만 하면 되므로 이 문제 자체가 없어진다. 옮길 때 "완료된 작업"엔 평소처럼 카테고리+키워드로 축약한다 — 문제가 해결되면 "왜 문제였는지"는 더 이상 필요 없어지기 때문(코드가 진실의 원천).
  - **(기존 레거시 항목)** 이 규칙이 생기기 전에 이미 분리해둔 항목(예: "빌드 전 반드시 고쳐야 하는 것")은 기존 방식 유지: "남은 작업"엔 짧은 참조만, 상세 내용은 별도 섹션에. 상세 섹션 쪽엔 역참조 표시(`(완료 시 이 섹션 삭제 — "OrcKing 보스 - 남은 작업"에서 참조됨)`)를 남겨서 참조 줄이 지워져도 삭제 대상이라는 걸 놓치지 않게 한다. 완료 처리할 때 상세 섹션을 바로 지우지 말고, 먼저 사용자에게 "이거 연결된 상세 섹션 있는데 같이 지울까?"라고 확인받은 뒤 처리한다.
- **이 파일을 수정할 때는 항상 Edit으로 해당 줄만 추가/수정하고, 파일 전체를 다시 쓰지 않는다.**
...
### OrcKing 보스 - 남은 작업 (미해결)
- **오크킹 스킬 사용 패턴 (수정 필요함) (진행중)**: 거리 기반(가까우면 Breath, 멀면 Jumping) 대신 시간 기반(10초 쿨타임, `farSkillWeight`로 Jumping 60%/Breath 40% 가중치 랜덤, 쿨타임은 스킬이 끝난 시점부터 다시 시작)으로 코드 수정 완료. **근데 테스트해보니 플레이어가 가만히 있으면 근접 공격만 계속 반복되고 샤우트 패턴(Breath/Jumping)이 아예 안 나가는 버그 발견** — `Boss_Move.Update()`가 사거리 안에서 근접 공격이 끝나자마자 바로 `EnterAttack()`을 다시 걸어서 `isAttacking`이 계속 true로 유지되고, 그래서 `Monster_State_Boss.Update()`의 샤우트 체크 로직이 실행될 틈이 없어지는 것으로 추정. **다음 세션에 우선 해결할 것.**
- **플레이어 피격 처리 구현**: `Player.cs`의 `MonsterAttack` 레이어 `OnTriggerEnter`가 아직 빈 스텁(케이스만 있고 실제 처리 없음)이라, 지금은 보스 공격이 다 맞아도 플레이어 체력이 안 깎인다. `Monster_HitDamage`가 플레이어 공격 받는 방식(레이어 감지 → 상대방 컴포넌트의 데미지 계산 함수 호출 → 자기 체력 차감, "데미지 전달 패턴" 참고)과 동일하게 구현할 것. **타격 파티클 작업(아래 "프로젝트 남은 작업") 끝난 뒤에 진행** — 데미지부터 넣으면 대상이 죽어버려서 파티클 확인할 시간이 줄어듦.
- **빌드: CSV 데이터 로드 경로 수정**: `Resources` 방식이 에디터에서만 동작하고 실제 빌드에서 깨짐. `StreamingAssets` 전환 또는 `Resources.Load<TextAsset>()`로 교체 필요 — 상세 원인/해결법은 "빌드 전 반드시 고쳐야 하는 것" 섹션 참고.
- **NPC 대화 시스템 구현 착수**: 테이블(CSV) 기반, 퀘스트/보스 진행 상태에 따라 조건부로 대화 전환. 착수 시 자세한 설계 논의 필요.

### OrcKing 보스 - 완료된 작업
- 보스 (09/15/26): Slash Attack(검풍) 포기, 제자리 점프+지면 스파이크 이펙트 패턴으로 교체

## 프로젝트 남은 작업

OrcKing 전용이 아니라 프로젝트 전반에 걸친 작업. 형식은 위 "작업 로그 규칙"과 동일.

### 남은 작업
- **타격(피격) 파티클 추가 — 몬스터/플레이어/보스 전체**: 지금은 발사/스윙 이펙트만 있고, 실제로 맞는 순간의 임팩트 이펙트는 따로 없음. **플레이어 피격 처리 구현보다 먼저 할 것** — 데미지 처리가 먼저 들어가면 맞는 대상이 죽어서 파티클을 오래 확인할 기회가 줄어드므로, 파티클부터 붙이고 충분히 테스트한 뒤 데미지 넣는 순서로.
...
*(완료 시 이 섹션 삭제 — "OrcKing 보스 - 남은 작업"에서 참조됨)*
...
**`BossEffectManager` 패턴 이름 매칭도 같은 종류의 함정이다.** `BossEffectManager`의 **직속 자식 오브젝트 이름**(예: "Breath", "SlashAttack")이 `PlayEffect("이름", ...)` 호출 시 넘기는 문자열과 **정확히 일치**해야 한다 — 대소문자, 공백까지 전부. 어긋나면 예외 없이 `Debug.LogWarning`만 찍고 이펙트가 아예 안 나간다(콘솔 확인 안 하면 원인 파악 어려움). 특히 외부에서 가져온 프리팹(믹사모 등)을 그대로 자식으로 끌어다 놓으면, 프리팹 원본 이름에 실수로 공백이 붙어있거나 코드가 기대하는 이름과 다를 수 있으니 **오브젝트 이름을 코드 문자열에 맞춰 깔끔하게 바꿔줄 것** (반대로 코드 문자열을 프리팹 이름에 맞추지 말 것 - 오타/공백이 그대로 코드에 박혀서 더 취약해짐). 그 안쪽(자식의 자식 등, 실제 `Boss_Attack_Effect` 컴포넌트가 붙은 오브젝트)의 이름은 `GetComponentInChildren`으로 찾으므로 상관없다 - **직속 자식 하나만** 이름이 중요하다.
